...
export default function Home() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("https://agentify.jp/openapi/agents/chat/stream/v1", {
        method: "POST",
        headers: {
        ...
        for (const part of parts) {
          if (part.startsWith("data:")) {
            ...
            try {
              ...
              if (obj.content) currentText += obj.content;
              if (obj.complete === true) {
                const blocks = currentText.match(/```json[\s\S]*?```/g);
                if (!blocks) {
                  alert("⚠️ JSONブロックが見つかりませんでした");
                  return;
                }

                blocks.forEach((block, i) => {
                  const raw = block.replace(/```json|```/g, '').trim();
                  try {
                    const parsed = JSON.parse(raw);
                    resultsRaw.push(parsed);
                  } catch (e) {
                    console.warn(`⚠️ JSON解析失敗 (${i + 1})，内容:`, raw);
                  }
                });

                setResults(resultsRaw.map((r, i) => ({ filename: images[i]?.name || `file-${i}`, result: r })));
                setSelected(resultsRaw.map(() => true));
              }
            } catch (e) {
              console.warn("⚠️ JSON片段解析失敗：", e);
            }
          }
        }
      }
                ...
                <tbody className="bg-white">
                  {results.map((r, i) => (
                    <tr key={i} className="border-t hover:bg-gray-50">
                      ...
                      <td className="px-2 py-2 break-words whitespace-pre-wrap">{r.result['取引先名']}</td>
                      <td className="px-2 py-2 break-words whitespace-pre-wrap">{r.result['支出明細']}</td>
                      <td className="px-2 py-2 whitespace-nowrap">{r.result['金額（JPY)']}</td>
                      <td className="px-2 py-2">
